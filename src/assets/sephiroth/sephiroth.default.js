...
var initialize = (source, markInfo) => {
  ...
  if (source != null) {
    ...
    img.onload = function() {
      ...
      console.info(
        'origianl image width and height:' + imgWidth + ',' + imgHeight
      )
...
var initImage = () => {
  ...
  var zoomDiv =
    "<div id='zommDiv' style='position:absolute;left:10px;top:5px;background:#fffef9;width:90px;height:20px;z-index:9999'>级别：" +
    rateWidth +
    '% </div>'
...
export { sephiroth, set, getfunction }
...
;(function() {
  ...
  p.drawpoint = function() {
    ...
    for (var i = 0; i < this.e_data.length; i++) {
      ...
      tempp.graphics
        .beginFill('#F00')
        .drawCircle(this.e_data[i][0][0], this.e_data[i][0][1], 2)
    ...
    tempp.graphics
      .beginFill('#F00')
      .drawCircle(
        this.e_data[this.e_data.length - 1][1][0],
        this.e_data[this.e_data.length - 1][1][1],
        2
      )
    tempp.cursor = 'pointer'
    tempp.addEventListener('mousedown', function(evt) {
    ...
    this.evt_stageclick = function(evt) {
      if (p_this.point0.length == 0) {
        ...
        var temppressup = function(up_evt) {
          ...
          if (temptype == 1) {
            p_this.e_data.push([
              [p_this.point1[0], p_this.point1[1]],
              [t_x, t_y]
            ])
          } else {
            p_this.e_data.push([
    ...
    this.evt_winkeydown = function(evt) {
      if (evt.keyCode == 8) {
        ...
        var rl_linsten = function() {
          ...
          var cleave = function() {
            p_this.e_shape.alpha = e_color.ealpha
          }
...
;(function() {
  ...
  p.drawpoint = function() {
    ...
    _temppoint1.graphics
      .beginFill('#F00')
      .drawCircle(this.e_data[0], this.e_data[1], 4)
    var _temppoint2 = new createjs.Shape()
    _temppoint2.graphics
      .beginFill('#F00')
      .drawCircle(this.e_data[0] + this.e_data[2], this.e_data[1], 4)
    var _temppoint3 = new createjs.Shape()
    _temppoint3.graphics
      .beginFill('#F00')
      .drawCircle(this.e_data[0], this.e_data[1] + this.e_data[3], 4)
    var _temppoint4 = new createjs.Shape()
    _temppoint4.graphics
      .beginFill('#F00')
      .drawCircle(
        this.e_data[0] + this.e_data[2],
        this.e_data[1] + this.e_data[3],
        4
      )
  ...
  p.setup = function() {
    ...
    this.evt_pressmove = function(evt) {
      ...
      lg.rect(
        p_point[0],
        p_point[1],
        evt.stageX - p_point[0],
        evt.stageY - p_point[1]
      )
      lg.endStroke()
    }
